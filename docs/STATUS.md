...
## Work Log (per task)
- 2026-08-30 — WA notifications **Phase N.5 (templates) + N.7 (tier 2/3)** implemented. **N.5:** `apps/notifications/templates_registry.py` (event→`Template`, aliases, `get_template`, `template_mode`); `KirimChatBackend`/`send_whatsapp`/`deliver_whatsapp`/`dispatch` thread an optional `template` + `wa_params`; handlers/reminders/OTP pass params; `Setting WA_TEMPLATE_MODE` = off (default, plain text) / on / strict; `python manage.py wa_templates` lists ~14 templates to submit. kirim.chat template payload shape is a best-guess from the docs summary — **verify before setting WA_TEMPLATE_MODE=on**. **N.7:** `dispatch_low_balance_alerts()` (D-5..D-7, added to hourly `dispatch_all_reminders`); welcome message on first paid order (`handle_order_paid`); quiet hours (`dispatch._quiet_hours_delay`, `Setting WA_QUIET_START`/`WA_QUIET_END`, default 0/0 = off, urgent events exempt); signed one-click unsubscribe (`apps/notifications/unsubscribe.py` + `/notifications/unsubscribe/<token>/` + template; footer appended to single-channel emails & promo); `dispatch.notify_promo()` + `notifications.broadcast_promo` task (email-only, opt-in `notif_promo`). Tests: `tests/test_wa_templates_and_prefs.py`; `test_notifications.py` order-paid test updated for the welcome message. Suite: 267 pass (2 pre-existing failures). **Remaining:** submit + approve WABA templates then `WA_TEMPLATE_MODE=on`; deploy; win-back/new-product campaigns; auto-expire QRIS Statis orders (billing decision).
- 2026-08-30 — WA notifications **Phase N.6** (new Tier-1 reminders) implemented. `apps/notifications/reminders.py` refactored (`_send_once()` / `_window()` helpers; `dispatch_all_reminders()`). New reminders, all on the customer's effective channel, dedup via `NotificationLog`: **expiry** for non-auto-renew ACTIVE subs (D-7/D-3/D-1, no balance condition), **grace countdown** (D-2/D-1 before `period_end + SUBSCRIPTION_GRACE_DAYS` suspension), **pending-payment nudge** for PENDING qris_static orders (~2h / ~24h old). Hourly task `notifications.send_renewal_reminders` now calls `dispatch_all_reminders()` (no new beat entry). Deferred: auto-expiring QRIS Statis orders + "order expired" notice (needs a billing decision — no `Order.expires_at` today). Tests: `tests/test_lifecycle_reminders.py` (8). Suite: 258 pass (2 pre-existing failures). **Next: N.5** (WABA templates) then N.7 (low-balance/welcome, preference center).
- 2026-08-30 — WA notifications **Phase N.4** (OTP number verification) implemented. `WhatsAppOTP` model (hash-only, 5-min TTL, max 5 attempts; migration `notifications/0004`). `apps/notifications/otp.py` — `request_code()` / `verify_code()` with rate limits (60s cooldown, 3/hour/number, suppression check). Dashboard: `wa_send_otp` / `wa_verify_otp` views + URLs; `profile.html` reworked — WhatsApp number now set **only** via the OTP card (removed from the main profile form, which now handles just channel + promo). `verify_code` sets `wa_number` + `wa_number_verified_at`. Tests: `tests/test_wa_otp.py` (happy path, brute-force lock, expiry, cooldown, suppressed, view wiring); `test_dashboard.py` profile tests updated. Suite: 251 pass (2 pre-existing failures). **Next: N.5** (WABA templates) then N.6 (new Tier-1 notifications).
- 2026-08-29 — WA notifications **Phase N.2 + N.3** implemented (see [27-whatsapp-notifications.md](27-whatsapp-notifications.md) §E). `apps/notifications/dispatch.py` — `notify()` / `notify_wa_copy()` / `effective_channel()` / `fallback_delivery_to_email()`; `handlers.py` + `reminders.py` route through it. New model **`NotificationDelivery`** (outbox: event/channel/status/provider/provider_msg_id/error + retained wa_text/email_subject/email_body for fallback) + **`WhatsAppSuppression`** (migration `notifications/0003`). `deliver_whatsapp` task takes `delivery_id`, records sent/failed, fires email fallback when retries exhausted. Webhook `POST /notifications/webhook/kirimchat/` (`apps/notifications/webhooks.py` + `views.py` + `urls.py`, wired in `config/urls.py`): HMAC-SHA256 `X-KirimChat-Signature` verify, `event_id` idempotency via cache, `message.sent/delivered/read` → status, `message.failed` → email fallback, inbound `STOP`/`START` → `WhatsAppSuppression` + channel reset. `whatsapp.py`: fixed latent `Setting` NameError in `get_backend()`; backends now return `provider_msg_id`; added `wa_suppressed()`. **All notification text switched to English** (per DECISIONS.md Language rule) — handlers, reminders, webhook replies; affected test assertions updated. New tests: `tests/test_notification_webhook.py`. Suite: 244 pass (2 pre-existing failures unrelated). Admin: `WhatsAppSuppression` + `NotificationDelivery` registered. **Next: N.4** (WA number OTP verification + preference center), N.5 (WABA templates).
- 2026-08-29 — WA notifications **Phase N.1** implemented (see [27-whatsapp-notifications.md](27-whatsapp-notifications.md) §E). `apps/core`: `NotificationChannel` enum + `apps/core/validators.py` (`normalize_wa_number` / `validate_wa_number`). `Customer`: `notification_channel` + `wa_number_verified_at` + `notif_promo`, `resolve_channel()` / `wa_verified` / `notify_email_address`; `notif_wa`/`notif_email` removed; migration `accounts/0007_customer_notification_channel` (schema + data: `notif_wa & not notif_email` → `whatsapp`, else `email`; `wa_number` normalized/cleaned; all `wa_number_verified_at` NULL → WA users fall back to email until re-verify). `KirimChatBackend` in `notifications/whatsapp.py` (`WA_BACKEND=kirimchat`, `WA_TOKEN` env). `handlers.py` + `reminders.py` → single channel via `resolve_channel()` (receipts/orders always emailed + WA copy if chosen; `reminders` dedup_key now one per (sub,window)); seller `order.awaiting_confirmation` moved WA→email. `CustomerAdmin` + `dashboard/profile` form updated (channel radio, WA locked until verified, promo toggle). Tests: new `tests/test_notification_channel.py`; `tests/test_notifications.py` rewritten for one-channel model — 224 pass (2 pre-existing failures unrelated). `.env.example` + `docs/08`/`docs/23` updated. **Next: N.2** (unified `notify()` + `NotificationLog`→outbox), N.3 (kirim.chat webhook + `WhatsAppSuppression`), N.4 (OTP).
- 2026-08-29 — PLAN written: [27-whatsapp-notifications.md](27-whatsapp-notifications.md) + **ADR-022** (decisions final). Adopts **kirim.chat** as WA gateway (closes "WA gateway" open question). Core change: customer notifications become a **single channel choice** (email XOR whatsapp) instead of two additive booleans — `Customer.notif_wa`/`notif_email` → `notification_channel` + `wa_number_verified_at` + `notif_promo` (on `Customer` only). **Sellers stay email-only** (`SellerProfile` unchanged; WA-to-seller code removed). Value docs always emailed. Promo email-only for now. Also: unified `notify()` dispatch; `KirimChatBackend` + webhook (delivery status + STOP → `WhatsAppSuppression`); WA number OTP verification; WABA utility-template registry; `NotificationLog` → delivery/outbox. Not yet implemented — Phase N.1 next.
- 2026-08-28 — Payment gateway swapped Duitku → Sumopod (ADR-021). New `apps/billing/sumopod.py` (SumopodClient: create_payment, verify_webhook via Svix HMAC + X-Webhook-Token). `services.process_webhook_payload` reshaped for Sumopod events (payment.completed/failed/expired/test); verification moved to the view. `views.sumopod_webhook` at `/billing/webhook/sumopod/`, idempotency key `sumopod:<svix-id>`, 200/401/400/500 responses. `duitku_client` kwarg → `gateway_client` across checkout/cart/services. Payment-method picker removed (QRIS-only + maintenance notice). Safety-net now expire-only (no status endpoint). `TopUp.Gateway.SUMOPOD` default + migration `0010`. Env: `SUMOPOD_API_KEY` / `SUMOPOD_WEBHOOK_SECRET` / `SUMOPOD_WEBHOOK_TOKEN` / `SUMOPOD_SANDBOX` (secrets env-only). Tests rewritten (test_topup/checkout/cart/storefront). `apps/billing/duitku.py` deleted. Follow-ups: `_post` sends a browser `User-Agent` (Cloudflare error 1010 fix); webhook amount cross-check accepts `net_amount` (fee passthrough); `SumopodError` handled in checkout/cart views (no more 500); fee-to-customer surfaced in checkout/top-up/cart UI + `estimate_fee()` + `TopUp.gateway_fee` persisted from the create_payment response (migration `0011`).
...
- [x] **WA gateway** — RESOLVED: **kirim.chat**, customer-only, single-channel choice. See [ADR-022](DECISIONS.md) + [27-whatsapp-notifications.md](27-whatsapp-notifications.md). Implementation pending (Phase N.1+).
